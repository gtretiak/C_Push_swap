...
do
	...
	(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes $ROOT/push_swap $(cat $TRACE/error_files/test_case_$i.txt) 1>a 2>b) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
	wait $pid 2>/dev/null;
	grep "0 errors from 0 contexts" b > x
	grep "not found" b > y
	if [[ -s x ]];
	then
...
printf "test_case_16\n\n"
	...
	rm -rf stdout stderr
	(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes $ROOT/push_swap "123 1 2" "654 456 123" 1>a 2>b) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
	wait $pid 2>/dev/null;
	grep "0 errors from 0 contexts" b > x
	grep "not found" b > y
...
do
	...
	rm -rf stderr stdout
	(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes $ROOT/push_swap "$(cat $TRACE/identity_files/test_case_$i.txt)" 1>a 2>b) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
	wait $pid 2>/dev/null;
	grep "0 errors from 0 contexts" b > x
	grep "not found" b > y
...
do
	...
	(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes $ROOT/push_swap "$(cat $TRACE/size3/test_case_$i.txt)" 1>a 2>b) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
	wait $pid 2>/dev/null;
	grep "0 errors from 0 contexts" b > x
	grep "not found" b > y
...
do
	...
	rm -rf stdout stderr result error_result ok aa
	(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes $ROOT/push_swap "$(cat $TRACE/size5/test_case_$i.txt)" 1>a 2>b) & pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
	wait $pid 2>/dev/null;
	grep "0 errors from 0 contexts" b > x
	grep "not found" a > y
